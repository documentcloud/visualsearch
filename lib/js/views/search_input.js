...
VS.ui.SearchInput = Backbone.View.extend({
  ...
  addTextFacetRemainder : function(facetValue) {
 	...
 	if (this.isFacetExists(facetValue)) return;
  ...
  // when autocomplete selected, avoids TEXT facet to be created as query 
  isFacetExists : function (facet) {
    var facetExists = this.options.app.options.callbacks.facetMatches(function(prefixes) {
	  prefixes = prefixes || [];
	      
	  var matcher    = new RegExp('^' + facet, 'i');
	  var matches    = $.grep(prefixes, function(item) {
	    return item && matcher.test(item.label || item);
	  });
	  ...
	  if (matches.length==0) {
	    var matches    = $.grep(prefixes, function(item) {
	      return item && matcher.test(item.value || item);
		});	    	  
      }
  ...
  keypress : function(e) {
    ...
    if (key == 'enter') {
      ...
      var prefixes = this.app.options.callbacks.facetMatches(function (prefixes_options) {return prefixes_options;});
      var labels   = _.map(prefixes, function(prefix) {
        if (prefix.label) return prefix.label;
        else              return prefix;
      });
      var values   = _.map(prefixes, function(prefix) {
        if (prefix.value) return prefix.value;
        else              return false;
      });
      labels = $.merge(labels, _.compact(values));
