  ...
  VS.ui.SearchFacet = Backbone.View.extend({
    ...
    setupAutocomplete: function() {
      ...
      this.box.autocomplete('widget').addClass('VS-interface ' +
        this.options.facetAutocompleteClassName);
    },
    ...
    autocompleteValues: function(req, resp) {
      ...
      var isItemString, re, matcher;

      this.app.options.callbacks.valueMatches(category, searchTerm, function(matches, options) {
        options = options || {};
        matches = matches || [];
        isItemString = _.isString(matches[0]);

        if (searchTerm && value != searchTerm) {
          if (options.preserveMatches) {
            resp(matches);
          } else {
            re = VS.utils.inflector.escapeRegExp(searchTerm || '');
            matcher = new RegExp('\\b' + re, 'i');
            matches = $.grep(matches, function(item) {
              return isItemString ? matcher.test(item) :
                matcher.test(item.value) || matcher.test(item.label);
            });
          }
        }

        // to prevent IE going to dream about death
        ...
        if (options.preserveOrder) {
          ...
          resp(matches.sort(function(a, b) {
            return isItemString ?
              (a > b ? 1 : -1) :
              (a.value > b.value ? 1 : -1);
          }));
    ...
    search: function(e, direction) {
      ...
      if (this.app.options.autosearch) {
        ...
        this.app.options.callbacks.formatFacets();
      }
    ...
    deferDisableEdit: function() {
      if (document.activeElement !== this.box.data('ui-autocomplete').menu.element.get(0)) {
    ...
    keydown: function(e) {
      ...
      if (this.app.options.enterKeyEnabled && (key == 'enter') && this.box.val()) {
        this.disableEdit();
        this.search(e);
      } else if (key == 'left') {
        if (this.modes.selected == 'is') {
